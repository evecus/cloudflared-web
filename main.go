...
var (
	...
	lastErr        string // 最近一次启动失败的原始错误信息，用于前端展示

	authUser = "admin"
	authPass = "" // 空字符串表示不启用鉴权
)

const (
	defaultPort = "12222"
	defaultBin  = "cloudflared"
	defaultDir  = "/app/data"
)
...
func startTunnelWithCheck(token string) bool {
	cmd := exec.Command(cloudflaredBin, "tunnel", "--no-autoupdate", "run", "--token", token)
	...
	case <-time.After(7 * time.Second):
		_ = cmd.Process.Kill()
		mu.Lock()
		lastErr = "timeout waiting for cloudflared to report a successful connection"
		mu.Unlock()
		return false
	}
}
...
func indexHandler(w http.ResponseWriter, r *http.Request) {
	...
	if msgKey != "" {
		switch msgKey {
		case "saved":
			displayMsg = locales[lang].SaveSuccess
		case "save_err":
			displayMsg = locales[lang].SaveFail
		case "conn_err":
			displayMsg = locales[lang].ConnFail
			mu.Lock()
			if lastErr != "" {
				displayMsg = displayMsg + " (" + lastErr + ")"
			}
			mu.Unlock()
		}
	}
...
// basicAuthMiddleware 在 authPass 非空时启用 HTTP Basic Auth 保护；
// authPass 为空（未指定 --password）时直接放行，保持向后兼容。
func basicAuthMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok || !userMatch || !passMatch {
			w.Header().Set("WWW-Authenticate", `Basic realm="Tunnel Manager"`)
...
func main() {
	portFlag := flag.String("port", defaultPort, "监听端口")
	binFlag := flag.String("bin", defaultBin, "cloudflared 二进制路径 (默认从 PATH 查找)")
	dirFlag := flag.String("dir", defaultDir, "数据存储目录")
	passwordFlag := flag.String("password", "", "启用 Basic Auth 鉴权的密码；不指定则不开启鉴权")
	flag.Parse()

	// 优先级：命令行参数（若与默认值不同，说明用户显式传了）> 环境变量 > 默认值
	port := *portFlag
	...
	cloudflaredBin = *binFlag
	if *binFlag == defaultBin {
		if v := os.Getenv("CLOUDFLARED_BIN"); v != "" {
			cloudflaredBin = v
		}
	}

	dataDir = *dirFlag
	if *dirFlag == defaultDir {
		if v := os.Getenv("DATA_DIR"); v != "" {
			dataDir = v
		}
	}
	tokenPath = strings.TrimRight(dataDir, "/") + "/token"

	// --password 未指定时，也允许通过环境变量 PASSWORD 设置（Docker/K8s secret 场景更安全）
	if *passwordFlag != "" {
		authPass = *passwordFlag
	...
	http.HandleFunc("/", basicAuthMiddleware(indexHandler))
	...
	println("cloudflared-web listening on", addr, "| bin:", cloudflaredBin, "| dir:", dataDir, "| auth:", authState)
